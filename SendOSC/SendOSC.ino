...
void loop() {
  ...
  // send packet:
  if ((currentMillis - sendInterval) > previousMillis) {
    counter++;
    String osc_message = "Hello from Teensy #" + String(counter);
    Serial.print("sending osc message: ");
    Serial.println(osc_message);

    // convert osc_message String to char array
    int str_len = osc_message.length() + 1; 
    char osc_message_char[str_len];
    osc_message.toCharArray(osc_message_char, str_len);
    
    OSCBundle bundleOut;
    bundleOut.add("/foo").add(osc_message_char);
